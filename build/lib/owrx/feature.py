...
class FeatureDetector(object):
    ...
    def _has_soapy_driver(self, driver):
        try:
            # Use SoapySDRUtil to list available drivers
            process = subprocess.Popen(["SoapySDRUtil", "--find"], stdout=subprocess.PIPE, stderr=subprocess.DEVNULL)
            
            output = process.communicate()[0].decode()
            process.wait()
            
            # Parse output for driver= lines
            import re
            drivers = re.findall(r'driver\s*=\s*(\w+)', output)
